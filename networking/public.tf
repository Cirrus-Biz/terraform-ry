...
resource "aws_route_table" "public_rt" {
 ...
 resource "aws_subnet" "aline-public2" {
  vpc_id = aws_vpc.mainvpc.id
  cidr_block = var.public_subnet

  tags = {
    Name = "public-subnet2-RY"
  }
}
...
resource "aws_route_table" "public_rt2" {
  ...
  tags = {
      Name = "public-route-table2-RY"
  }
}

    # Route table Association with Public Subnet's
 resource "aws_route_table_association" "aline_public_rt_association2" {
    subnet_id = aws_subnet.aline-public2.id
